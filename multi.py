    ...
    def inside_bar_breakout(df):
        # Mother (-4), 2 inside (-3, -2), breakout (-1)
        if len(df) < 4:
            return None
        ...
        # दोनों inside candles पूरी तरह mother के high/low के अंदर हों
        inside_ok = (
            inside1["high"] < mother["high"] and inside1["low"] > mother["low"] and
    ...
    def adx_expansion(df):
        ...
        if adx.iloc[-2] < 20 and adx.iloc[-1] > 23:
    ...
    if run:
        ...
        results = []
        atr_list = []  # सिर्फ इसी scanner के लिए
        ...
        for sym, df in data.items():
            ...
            if scanner == "Top 10 by ATR %":
                ...
                # sort by ATR% desc and take top 10
